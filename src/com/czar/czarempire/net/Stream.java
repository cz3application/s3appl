...
public class Stream {
	...
	private final int frameStackSize = 10;
	...
	public void expandOutBuffer() {
		 ...
		 */
		final byte[] oldBuffer = outBuffer;
	...
	public int r2() {
		return (inBuffer[inOffset++] & 0xff) | ((inBuffer[inOffset++] & 0xff) << 8)
				| ((inBuffer[inOffset++] & 0xff) << 16) | ((inBuffer[inOffset++] & 0xff) << 24);
	}
	...
	public int readDWord_v3() {
		inOffset += 4;
		return ((inBuffer[inOffset - 3] & 0xff) << 24) + ((inBuffer[inOffset - 4] & 0xff) << 16)
				+ ((inBuffer[inOffset - 1] & 0xff) << 24) + (inBuffer[inOffset - 2] & 0xff);
	}
	...
	public long readQWord2() {
		inOffset += 8;
		return (((inBuffer[inOffset - 8] & 0xff) << 56) + ((inBuffer[inOffset - 7] & 0xff) << 48)
				+ ((inBuffer[inOffset - 6] & 0xff) << 40) + ((inBuffer[inOffset - 5] & 0xff) << 32)
				+ ((inBuffer[inOffset - 4] & 0xff) << 24) + ((inBuffer[inOffset - 3] & 0xff) << 16)
				+ ((inBuffer[inOffset - 2] & 0xff) << 8) + (inBuffer[inOffset - 1] & 0xff));
	}
